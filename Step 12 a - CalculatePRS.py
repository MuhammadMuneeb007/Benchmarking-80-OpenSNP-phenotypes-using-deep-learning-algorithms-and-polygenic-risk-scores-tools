   ...
   for c1 in clump1:
    for c2 in clump2:
     for c3 in clump3:
      ...
      rs = pd.DataFrame()
      # Perform Quality control steps on the GWAS summary statistic file.
      gwasqualityplink(traindirec,fdirec) 
      testquality(testdirec,fdirec,x1,x2,x3,c1,c2,c3)
      plink(direc)
      rs['PlinkAUC'] = plinkAUC
      PRSice(fdirec,testdirec)
      rs['prsiceAUC'] = prsiceAUC
      changechromosomeforlasso() 
      lasso(direc)
      rs['lassoAUC'] = lassoAUC
      rs.to_csv(direc+os.sep+"ResultsPRS.txt",index=False)
