...
def calculateMCCPRScice(path,test):
  ...
  if all(item == 0 for item in best["PRS"].values) or all(item == 1 for item in best["PRS"].values):
   best["PRS"].values[best["PRS"] >=0.5] = 1
   best["PRS"].values[best["PRS"] < 0.5] = 0
   print(best["PRS"].values)
   print(pheno['f'].values)
   return matthews_corrcoef(best["PRS"].values, pheno['f'].values)
  else:
   best["PRS"] = best["PRS"].fillna(0)
   best["PRS"] = NormalizeData(np.array(best["PRS"].values))
   best["PRS"].values[best["PRS"] >=0.5] = 1
...
def PRSice(fdirec,testdirec):
  ...
  os.system("Rscript PRSice.R --prsice PRSice --base "+fdirec+"/Data.QC.gz --target "+fdirec+"/test.QC  --thread 1 --print-snp --stat OR --binary-target T --interval 5e-01 --upper 1.0 --no-clump --out "+result+os.sep+"PRScice_PRS")
   ...
   for c1 in clump1:
    for c2 in clump2:
     for c3 in clump3:
      ...
      PRSice(fdirec,testdirec)

      rs['PRSiceAUC'] = prsiceAUC
      rs2['PRSiceMCC'] = prsiceMCC


      rs.to_csv(direc+os.sep+"Results_PRSice_AUC.txt",index=False)
      rs2.to_csv(direc+os.sep+"Results_PRSice_MCC.txt",index=False)
      print(rs)
      print(rs2)
